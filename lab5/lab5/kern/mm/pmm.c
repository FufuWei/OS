 ...
 */
/* copy_range - 将进程A的内存内容（从start到end）复制到进程B

@to: 进程B的页目录地址
@from: 进程A的页目录地址
@share: 表示复制还是共享的标志。我们只使用复制方法，因此此参数未被使用。
调用图：copy_mm–>dup_mmap–>copy_range
负责将内存页的内容从一个进程复制到另一个进程
*/
//函数接受四个参数：to（目标页目录），from（源页目录），start（要复制的范围的起始地址），end（要复制的范围的结束地址）。
int copy_range(pde_t *to, pde_t *from, uintptr_t start, uintptr_t end,
               bool share) {
                //第一个断言语句用于确保start和end是页对齐的，即它们能够被页面大小（PGSIZE）整除。
    assert(start % PGSIZE == 0 && end % PGSIZE == 0);
    //第二个断言语句用于确保start和end在用户访问空间内
    assert(USER_ACCESS(start, end));
    // copy content by page unit.
    //使用一个循环来逐页复制内容
    do {
        // call get_pte to find process A's pte according to the addr start
        //在循环中，首先调用get_pte函数根据地址start在进程A的页表中找到对应的页表项（pte_t *ptep）
        pte_t *ptep = get_pte(from, start, 0), *nptep;
        //如果找不到页表项，说明该页表项所在的页表不存在，此时需要将start向下舍入到最近的页表边界，并继续下一次循环
        if (ptep == NULL) {
            start = ROUNDDOWN(start + PTSIZE, PTSIZE);
            continue;
        }
        //如果找到了页表项，继续执行下面的代码。
        //接下来，调用get_pte函数在进程B的页表中找到地址start对应的页表项（pte_t *nptep）。
        // call get_pte to find process B's pte according to the addr start. If
        // pte is NULL, just alloc a PT
        //如果找不到页表项，说明页表不存在，此时需要分配一个新的页表。
        if (*ptep & PTE_V) {
            ...
            // get page from ptep
            //使用pte2page函数从进程A的页表项中获取对应的页面（struct Page *page）
            struct Page *page = pte2page(*ptep);
            //为进程B分配一个新的页面（struct Page npage）
            // alloc a page for process B
            struct Page *npage = alloc_page();
            //对一些断言进行验证，确保页面和页表项都被正确地获取和分配。
            assert(page != NULL);
            assert(npage != NULL);
            int ret = 0;
             ...
             * (4) build the map of phy addr of  npage with the linear addr start
             复制页面的内容到npage，用线性地址start建立nage的物理地址映射。

一些有用的宏和定义，你可以在下面的实现中使用：
宏或函数：
page2kva（struct Page * page）：返回page管理的内存的内核虚拟地址（参见pmm.h）
page_insert：用线性地址la构建Page的物理地址映射
memcpy：典型的内存复制函数

（1）找到src_kvaddr：页面的内核虚拟地址
（2）找到dst_kvaddr：npage的内核虚拟地址
（3）从src_kvaddr复制内存到dst_kvaddr，大小为PGSIZE
（4）用线性地址建立nage的物理地址映射
             */
            //使用page2kva函数将page和npage转换为内核虚拟地址src_kvaddr和dst_kvaddr
            void *src_kvaddr = page2kva(page);
            void *dst_kvaddr = page2kva(npage);
            //使用memcpy函数将进程A的页内容复制到进程B的页中
            memcpy(dst_kvaddr, src_kvaddr, PGSIZE);
            //使用page_insert函数将新的物理页npage插入到进程B的页表中
            ret = page_insert(to, npage, start, perm);
            assert(ret == 0);
        }
        //将start增加一个页大小，继续下一个页的复制
        start += PGSIZE;
    } while (start != 0 && start < end);
    return 0;
}
